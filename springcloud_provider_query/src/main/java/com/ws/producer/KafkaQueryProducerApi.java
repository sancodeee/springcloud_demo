...
import com.ws.utils.TypeConverUtil;
...
public class KafkaQueryProducerApi {
    ...
    public String queryAllProducer() {
        kafkaTemplate.send(TOPIC_QUERY, 0, "query_all", "");
    ...
    public List<Book> queryAllReturn(ConsumerRecord<?, ?> record, Acknowledgment ack) {
        ack.acknowledge();//手动确认
        Optional<?> value = Optional.ofNullable(record.value());
        if (value.isPresent()) {
            Object o = value.get();
            List<Book> bookList = TypeConverUtil.objToList(o, Book.class);
            return bookList;
        } else {
            log.error("对象不能为空");
            return null;
